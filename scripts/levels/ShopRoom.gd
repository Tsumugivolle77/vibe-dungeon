...
func _build_shop():
	var centre = get_player_start()   # room-local centre

	# Banner (room-local position so it lands correctly in offset branch rooms)
	...
	banner.position = centre + Vector2(-150, -150)
	add_child(banner)

	# 2 random weapons + health pack + ammo pack. Weapon prices scale with power.
	...
	var items = []
	items.append({"kind": "weapon", "id": common_ids[0], "price": WeaponDatabase.weapon_price(common_ids[0])})
	items.append({"kind": "weapon", "id": common_ids[1], "price": WeaponDatabase.weapon_price(common_ids[1])})
...
func _make_shop_node(item: Dictionary, pos: Vector2) -> Node2D:
	...
	container.position = pos   # room-local; global_position resolves via room transform
	container.set_meta("shop_data", item)
	# Weapon items also expose their id for the HUD stat tooltip.
	if item.kind == "weapon":
		container.add_to_group("weapon_display")
		container.set_meta("weapon_id", item.id)
